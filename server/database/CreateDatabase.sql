...
CREATE TABLE `events` (
	...
	`associated_festival` 	    int unsigned 	    NOT NULL 						COMMENT 'The ID of the associated festival.',
	`associated_artist` 	    int unsigned 	    DEFAULT NULL 					COMMENT 'The ID of the associated artist.',
	`associated_location` 	    int unsigned 	    DEFAULT NULL 					COMMENT 'The ID of the associated location.',
 
PRIMARY 	KEY (`event_id`),
FOREIGN 	KEY (`associated_festival`)		REFERENCES festivals (festival_id) 	ON DELETE CASCADE 	ON UPDATE CASCADE,
FOREIGN 	KEY (`associated_artist`) 		REFERENCES artists (artist_id) 		ON DELETE SET NULL 	ON UPDATE SET NULL,
FOREIGN 	KEY (`associated_location`) 	REFERENCES locations (location_id) 	ON DELETE SET NULL 	ON UPDATE SET NULL
...
CREATE TABLE `places` (
	 ...
	 `place_country` 			varchar(65) 		NOT NULL DEFAULT 'DE'			COMMENT 'The country of the place.',
